...
$mvcorex = mssql_select_db($db_name_multi,$chup);
...
//------------------------------------------------------------------
// -== Check VIP ==- //
$quardy = mssql_query('SELECT memb___id, mvc_vip_date from '.$mvcore_medb_i.' where memb___id = "'.$_SESSION['username'].'"');
$quary_drop = mssql_fetch_row($quardy);

if( $quary_drop[1] > time()){ // VIP SYS
	$mvcore['vip_sys_active'] = '1';
} else {
	$mvcore['vip_sys_active'] = '0';
	$edit_site = mssql_query("UPDATE ".$mvcore_medb_i." SET mvc_vip_date = '0' WHERE memb___id = '".$_SESSION['username']."'");
};
//------------------------------------------------------------------
...
if(isset($_POST['loginacc'])) {
	$send_login = checklogin($_POST['usern'],$_POST['passn'],$mvcore['md5_support'],$mvcore_medb_i,$mvcore['go_to_page'],$_GET['op1'],$mvcore['md5_hash_o'],$mvcore['md5_hs_salt']);
	
	if($send_login == '1'){ header('Location: -id-Login-id-er1.html'); }
	if($send_login == '2'){ header('Location: -id-Login-id-er2.html'); }
	...
	if($iMd5 == 'yes') {  //MD5
		...
		if($get_user == '' && $getpass == '') { return 1;
				...
				if($mdformat == 'dbo_fn_md5') {
					$quardys = mssql_query("SELECT memb__pwd, memb___id, mvc_vip_date,smtp_block from ".$nmedb_data." where memb___id = '".$get_user."' and memb__pwd = ".$pass_hash."");
					...
					$quardys = mssql_query("SELECT memb___id, mvc_vip_date, smtp_block from ".$nmedb_data." where memb___id = '".$get_user."'");
					$quary_dropsa = mssql_fetch_row($quardys);
					
					$sqll= mssql_query("declare @mdpasscheck varbinary(16); set @mdpasscheck = (select memb__pwd from ".$nmedb_data." where memb___id='".$get_user."'); print @mdpasscheck;"); $mdPass = mssql_get_last_message(); $mdPass = substr($mdPass,2);
		...
		if($get_user == '' && $getpass == '') { return 1;
				...
				$quary = mssql_query("SELECT memb__pwd, memb___id, mvc_vip_date,smtp_block from ".$nmedb_data." where memb___id = '".$get_user."'");
				$quary_drop = mssql_fetch_row($quary);
				
				if($quary_drop[3] == '1') {} else {
...
//------------------------------------------------------------------
// -== Check Admin Access ==- //
function checkAdmin($user,$nmedb_data){
	...
	if($user == '') { echo''; } else {
		$quary = mssql_query("SELECT admincp from ".$nmedb_data." where memb___id = '".$user."'");
		$quary_drop = mssql_fetch_row($quary);
			if ($quary_drop[0] == '1'){
...
function checkGM($user,$nmedb_data){
...
// -== Check Login Admin, User, Gm & Logout If Doesnt Exist In DB Anymore. ==- //
	...
	if($userrjtrejej == '') { } else {
		$quarysd = mssql_query("SELECT admincp from ".$mvcore_medb_i." where memb___id = '".$userrjtrejej."'");
		$quary_dropwge = mssql_fetch_row($quarysd);
...
//------------------------------------------------------------------
// -== Account Info Send ==- //
$check_acc_logind = mssql_query("select acc_info_text from ".$mvcore_medb_i." where memb___id = '".$_SESSION['username']."'");
$check_acc_logind_out = mssql_fetch_row($check_acc_logind);
...
$realtime = time();
$banselectsa = mssql_query("Select name,type,unban_date from MVCore_Banned_PPL where unban_date < ".$realtime."");
...
} else { $run_update = mssql_query("Update memb_stat set connectstat = '0', ServerName = 'MVCore' where ServerName = 'fakeserv'"); };
//------------------------------------------------------------------
// -== Friend System Reward Script ==- //
if($_SESSION['user_login'] == 'ok') {
	$dctm_selcds = mssql_query("SELECT top ".$mvcore['friend_sys_limit']." friend_uid,to_who_uid,date,friend_rewarded from MVCore_Friend_List where to_who_uid = '".$_SESSION['username']."' order by date desc");
	for($i=0;$i < mssql_num_rows($dctm_selcds);++$i) {
	$rowsfa = mssql_fetch_row($dctm_selcds);
...
if($mvcore['vote_top_voters'] == '0' || $mvcore['vote_top_voters'] == '') {} else{
	...
	if($Conv_date1 >= $Conv_date2) {
		...
		if($mvcore['vote_top_rw_type'] == '1') { $run = mssql_query("Update ".$mvcore['credits_table']." set ".$mvcore['credits_column']." = ".$mvcore['credits_column']." + ".$rewrite_reward." where ".$mvcore['user_column']." IN ( SELECT TOP ".$winner_count." username COLLATE DATABASE_DEFAULT FROM MVCore_Vote_Log GROUP BY username ORDER BY SUM(votes) DESC )"); }
		elseif($mvcore['vote_top_rw_type'] == '2') { $run = mssql_query("Update ".$mvcore['credits_table']." set ".$mvcore['credits2_column']." = ".$mvcore['credits2_column']." + ".$rewrite_reward." where ".$mvcore['user_column']." IN ( SELECT TOP ".$winner_count." username COLLATE DATABASE_DEFAULT FROM MVCore_Vote_Log GROUP BY username ORDER BY SUM(votes) DESC )"); }
		elseif($mvcore['vote_top_rw_type'] == '3') {
			$run = mssql_query("Update ".$mvcore['credits_table']." set ".$mvcore['credits_column']." = ".$mvcore['credits_column']." + ".$rewrite_reward." where ".$mvcore['user_column']." IN ( SELECT TOP ".$winner_count." username COLLATE DATABASE_DEFAULT FROM MVCore_Vote_Log GROUP BY username ORDER BY SUM(votes) DESC )");
			$run = mssql_query("Update ".$mvcore['credits_table']." set ".$mvcore['credits2_column']." = ".$mvcore['credits2_column']." + ".$rewrite_reward." where ".$mvcore['user_column']." IN ( SELECT TOP ".$winner_count." username COLLATE DATABASE_DEFAULT FROM MVCore_Vote_Log GROUP BY username ORDER BY SUM(votes) DESC )");		
		};
		//END Special

				$run = mssql_query("delete from MVCore_Vote_Log");
...
$db_get_ms = mssql_query("select msponsor_limit, msponsor_date from ".$mvcore_medb_i." where memb___id = '".$_SESSION['username']."'");
$db_out_ms = mssql_fetch_row($db_get_ms);
$t_DayIs = date("m.d.y",time()); 

if($db_out_ms[1] < $t_DayIs) {
	// Setup new limit & new date day.
	$runUp = mssql_query("update ".$mvcore_medb_i." set msponsor_limit = '".$mvcore['ms_day_limit']."', msponsor_date = '".$t_DayIs."' where memb___id = '".$_SESSION['username']."'");
};
//------------------------------------------------------------------
// -== Get User IP ==- //
function getUserIP()
...
if($mvcore['gs_port'] == '') { $fn_serverStatus_GS = '<font color="red">'.gs_status_offline.'</font>'; } else { $fn_serverStatus_GS = chkServer($mvcore['gs_port']); };
if($mvcore['gs2_port'] == '') { $fn_serverStatus_GS2 = '<font color="red">'.gs_status_offline.'</font>'; } else { $fn_serverStatus_GS2 = chkServer($mvcore['gs2_port']); };
if($mvcore['js_port'] == '') { $fn_serverStatus_JS = '<font color="red">'.gs_status_offline.'</font>'; } else { $fn_serverStatus_JS = chkServer($mvcore['js_port']); };
if($mvcore['cs_port'] == '') { $fn_serverStatus_CS = '<font color="red">'.gs_status_offline.'</font>'; } else { $fn_serverStatus_CS = chkServer($mvcore['cs_port']); };

if(substr($fn_serverStatus_GS,0,20) == '<font color="green">') { $fn_serverStatus_GS_true_false ='1'; } else { $fn_serverStatus_GS_true_false ='0'; };
if(substr($fn_serverStatus_GS2,0,20) == '<font color="green">') { $fn_serverStatus_GS2_true_false ='1'; } else { $fn_serverStatus_GS2_true_false ='0'; };
if(substr($fn_serverStatus_JS,0,20) == '<font color="green">') { $fn_serverStatus_JS_true_false ='1'; } else { $fn_serverStatus_JS_true_false ='0'; };
if(substr($fn_serverStatus_CS,0,20) == '<font color="green">') { $fn_serverStatus_CS_true_false ='1'; } else { $fn_serverStatus_CS_true_false ='0'; };

function chkServer($port)
	{  
			if (!$x = @fsockopen('127.0.0.1', $port,$ERROR_NO,$ERROR_STR,(float)0.2)) // attempt to connect
			{
				$datse = '<font color="red">'.gs_status_offline.'</font>';
			}
			else
			{
				$datse = '<font color="green">'.gs_status_online.'</font>';
				if ($x)
				{
...
// -== Users Online ==- //
	$select_onln_usrs= mssql_query("SELECT count(*) FROM ".$mvcore_medb_s." WHERE ConnectStat = '1'");
	$mvc_Users_online = mssql_result($select_onln_usrs, 0, 0);
	
// -== Account Credits ==- //
	$select_cred_checker= mssql_query("Select ".$mvcore['credits_column'].",".$mvcore['credits2_column']." from ".$mvcore['credits_table']." where ".$mvcore['user_column']."='".$_SESSION['username']."'");
	$mvc_Money_credits= mssql_fetch_row($select_cred_checker);
	if($mvc_Money_credits[0] == ''){ $mvc_Money_credits = '0'; } else { $mvc_Money_credits = $mvc_Money_credits[0]; };
	
// -== Account Gold Credits ==- //
	$select_cred_chaseck= mssql_query("Select ".$mvcore['credits_column'].",".$mvcore['credits2_column']." from ".$mvcore['credits_table']." where ".$mvcore['user_column']."='".$_SESSION['username']."'");
	$mvc_Money_goldcredits= mssql_fetch_row($select_cred_chaseck);
	if($mvc_Money_goldcredits[1] == ''){ $mvc_Money_goldcredits = '0'; } else { $mvc_Money_goldcredits = $mvc_Money_goldcredits[1]; };
	
// -== Account wCoins ==- //
	$select_charwc_infaso= mssql_query("Select memb_guid,acc_ip from ".$mvcore_medb_i." where memb___id='".$_SESSION['username']."'");
	$s_charwc_ias= mssql_fetch_row($select_charwc_infaso);
	if($mvcore['guid_column'] == 'memb_guid'){ $fix_wcoins_useras = $s_charwc_ias[0]; } else { $fix_wcoins_useras = $_SESSION['username']; };
	$v_f_hfwew= mssql_query("Select ".$mvcore['wcoins_column']." from ".$mvcore['wcoins_table']." where ".$mvcore['guid_column']."='".$fix_wcoins_useras."'");
	$mvc_Money_wcoins= mssql_fetch_row($v_f_hfwew);
	if($mvc_Money_wcoins[0] == ''){ $mvc_Money_wcoins = '0'; } else { $mvc_Money_wcoins = $mvc_Money_wcoins[0]; };
	
// -== Account Vault Zen ==- //
	$select_zen_chaseck= mssql_query("Select money from warehouse where accountid ='".$_SESSION['username']."'");
	$mvc_Money_VaultZen= mssql_fetch_row($select_zen_chaseck);
	if($mvc_Money_VaultZen[0] == ''){ $mvc_Money_VaultZen = '0'; } else { $mvc_Money_VaultZen = $mvc_Money_VaultZen[0]; };
	
// -== Total Account ==- //
	$total_accountsg = mssql_query("SELECT count(*) FROM ".$mvcore_medb_i."");
	$tot_acc_d =mssql_result($total_accountsg, 0, 0);
	$mvc_total_Accounts = $tot_acc_d;
	
// -== Total Characters ==- //
	$total_charactersg = mssql_query("SELECT count(*) FROM Character");
	$tot_char_d =mssql_result($total_charactersg, 0, 0);
	$mvc_total_Characters = $tot_char_d;
	
// -== Total Guilds ==- //
	$total_guildsg = mssql_query("SELECT count(*) FROM guild");
	$tot_guil_d =mssql_result($total_guildsg, 0, 0);
	$mvc_total_Guilds = $tot_guil_d;
//------------------------------------------------------------------
// -== 65k Stats Fix ==- //
...
function chOnlineImg($Name,$mvcore_medb_s,$mvcore_onoff_style)
{
	$getuseracc = mssql_query("SELECT AccountID FROM character WHERE name ='".$Name."'");
	$getuseraccs = mssql_fetch_row($getuseracc);

	$resvaln = mssql_query("SELECT ConnectStat FROM ".$mvcore_medb_s." WHERE memb___id = '".$getuseraccs[0]."'");
	$resultVal = mssql_fetch_row($resvaln);
...
//------------------------------------------------------------------
// -== Get Account Flag ==- //
function outMvcFlag($chname,$mvcore_medb_i) {
	$sql_accid = mssql_query("Select accountid from character where name = '".$chname."'");
	$sql_accid_out = mssql_fetch_row($sql_accid);
	$sql_flagc = mssql_query("Select mvc_flag from ".$mvcore_medb_i." where memb___id = '".$sql_accid_out[0]."'");
	$sql_flag_out = mssql_fetch_row($sql_flagc);
	if($sql_flag_out[0] != '') { $flag='<center><div align="center" title="'.$sql_flag_out[0].'" class="mvcoreflag mvcoreflag-'.$sql_flag_out[0].'"></div></center>'; } else { $flag='-'; };
	return $flag;
};
//------------------------------------------------------------------
