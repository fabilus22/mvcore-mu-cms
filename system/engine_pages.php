...
if($_GET['op1'] != 'admincp' && $mvcore['anon_sys'] == 'on') {
	...
	chdir($dir);
	array_multisort(array_map('basename', ($files = glob("*.*"))), SORT_NUMERIC, $files);
...
$event_msg_post = mssql_query("select top 3 title,message,game_master,expire_date from MVCore_Event_Post where expire_date >= '".time()."' order by expire_date desc");
...
if($_GET['op1'] == 'admincp' && $_SESSION['admin_panel'] == 'ok') {} else {
	if($mvcore['siege_show_countdown'] == 'yes') {
		...
		$cs_drop_datas = mssql_query("select Siege_End_Date From MuCastle_Data");
		$cs_drop_data = mssql_fetch_row($cs_drop_datas);
