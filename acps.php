...
if(isset($_POST['hopt'])) {
	if($_POST['hopt'] == 'na') { echo '0'; } else {
		$repl = str_replace('--','',$_POST['hopt']); $repl = str_replace(';','',$repl);
		$select_joh_info= mssql_query("Select joh_id, joh_val, joh_cost from MVCore_Wshopp_Harmony where joh_name='".$repl."'");
...
if(isset($_POST['s_option'])) {
	if($_POST['s_option'] == '254') { echo '0'; } else {
		$repl = str_replace('--','',$_POST['s_option']); $repl = str_replace(';','',$repl);
		$select_soc_info= mssql_query("Select sok_id, sok_name, type, sok_cost from MVCore_Wshopp_Socket where sok_id='".$repl."'");
		$check_soc_info = mssql_fetch_row($select_soc_info);
		echo ''.$check_soc_info[3].'';
		if(isset($_POST['s_option_c'])){ echo''.mssql_get_last_message().''; }; //Debug Data check
	}
exit;
};
...
if(isset($_POST['topvotersett'])) { // Vote Settings
	...
	if($has_error >= '1'){} else {
		...
		$data .="\$mvcore['vote_top_rw_type'] = \"".$POST[5]."\";\n";
		$data .="?>";
